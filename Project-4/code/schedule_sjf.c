...
int schedule_single_task()
{
    ...
    struct node* prev_of_minTime=NULL;
    struct node* minTime=temp;
    while(temp!=NULL)
    {
        if(temp->task->burst<minTime->task->burst)
        {
            minTime=temp;
            prev_of_minTime=prev_of_temp;
        }
        prev_of_temp=temp;
        temp=temp->next;
    }
    ...
    run(minTime->task,minTime->task->burst);

    current_time+=minTime->task->burst;
    total_trunaround_time+=current_time;
    number_of_tasks++;

    //after simulation, delete the node from the list
    /*
    there is an special case in the schedule_sjf
        when the shortest job is the head of list.
    In such case, the shortest job doesn't have a pre-node.
    */
    if(minTime==(*head_first_in_head))
        (*head_first_in_head)=(*head_first_in_head)->next;
    else
        prev_of_minTime->next=minTime->next;
